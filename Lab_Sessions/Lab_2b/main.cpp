...
int main() {
	...
	while (!inFS.eof()) {
		...
		getline(inFS, state);
		state = state.substr(0, state.size());
		if (!inFS.fail()) {
			temp.SetValue(state);
		}
	...
	for (i = 0; i < zipCodeState.size(); ++i) {
		...
		if (zipCodeState.at(i).GetKey() == zip) abbrev = zipCodeState.at(i).GetValue();
	}


	for (i = 0; i < abbrevState.size(); ++i) {
		// TODO: Using state abbreviation, find state name
		if (abbrevState.at(i).GetKey() == abbrev) state = abbrevState.at(i).GetValue();
	}
